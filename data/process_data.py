...
def clean_data(df):
    '''
        Transforms the data by cleaning
        Args: 
            df: merged dataframe
        Return:
    ...
    category_colnames = list(pd.Series(row).apply(lambda x: x.split('-')[0]))
...
def save_data(df, database_filename):
    '''
        Loads the dataframe to database
        Args:
            df: cleaned dataframe
            database_filename: database to store       
    '''  
